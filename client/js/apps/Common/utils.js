...
export function add_instance_label_no_color(document, container, palette, name, count, amount=0, obj_size) {
    // wrapper
    let wrapper = document.createElement("label");
    container.appendChild(wrapper);
    let wrapper_name = name.split(" ").filter(Boolean);
    if (amount == 0) {
        ...
        // nested
        wrapper_name.push(count[0]);
        var parent_id = "labelparent_"+wrapper_name.join("_");
        wrapper.setAttribute("id", parent_id);
    }
    wrapper.setAttribute("class", "btn btn-lg btn-info");
    wrapper.setAttribute("style", "text-align: left; width: 320px;");
    // instance label
    let label_span = document.createElement("span");
    wrapper.appendChild(label_span);
    ...
    if (amount == 0) {
        ...
        //For Chinese Version
        //label_span.innerHTML = name + " | 数量: " + "<b>"+amount+"</b>";
        //For English Version
        label_span.innerHTML = name + " | Amount: " + "<b>"+amount+"</b>";
        // create nested labels
        var child_ids = []
        //For Chinese version
        //var chinese_pos = name.indexOf("(")
        //For English version, we do not need the line above
        for (let i = 0; i < amount; ++i) {
            wrapper = document.createElement("label");
            container.appendChild(wrapper);
            let wrapper_name = name.split(" ").filter(Boolean);
            wrapper_name.push(count[i]);
            let child_id = "label_"+wrapper_name.join("_")
            child_ids.push(child_id);
            wrapper.setAttribute("id", child_id);
            ...
            wrapper.appendChild(label_span);
            label_span.setAttribute("style", "float: left;");
            //console.log(obj_size[i])
            //For English Version
            label_span.innerHTML = name + " " + obj_size[i].join(", ");
            //For Chinese Version
            //label_span.innerHTML = name.slice(0, chinese_pos) + " " + obj_size[i].join(", ");
        }

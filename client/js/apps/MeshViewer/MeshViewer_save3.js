...
class MeshViewer {
		...
		Promise.all(promises).then(values => {
			...
			Promise.all(new_promises).then(new_values => {
				...
				*/
				...
				const lineMaterial = new THREE.LineBasicMaterial({ color: 0x00ff00, opacity: 0.65, transparent: true, linewidth: 3});
				...
				/*
				// this.bounding_box_dragControls.addEventListener('drag', (event) => {
				this.bounding_box_dragControls.addEventListener('dragstart', (event) => {
					...
					if (event.pointerType == 'mouse' && event.button == 0) {
						...
						//console.log(`${this.bounding_box_dragControls.enabled}`)
				...
				*/
				...
				//const centerSphereGeometry = new THREE.SphereGeometry(0.1);
				//const centerSphereMaterial = new THREE.MeshPhongMaterial({ color: 0xff0000 , opacity: 0.65, transparent: true});
				...
				const edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(0.5, 0.5, 0.5));
				const lineMaterial = new THREE.MeshPhongMaterial({ color: 0x00ff00, opacity: 0.65, transparent: true});
				this.bounding_box = new THREE.LineSegments(edges, lineMaterial);

				this.bounding_box.position.set(boundingbox_Pos.x, boundingbox_Pos.y, boundingbox_Pos.z);
				console.log("hello", this.bounding_box.position)
				this.add_mesh(this.scene, [this.bounding_box])
				...
				*/
				...
				/*
				const xAxisMaterial = new THREE.MeshStandardMaterial({ color: 0xff0000 , emissive: 0xff0000 }); // X 轴颜色
				const yAxisMaterial = new THREE.MeshStandardMaterial({ color: 0x00ff00 , emissive: 0x00ff00}); // Y 轴颜色
				const zAxisMaterial = new THREE.MeshStandardMaterial({ color: 0x0000ff , emissive: 0x0000ff}); // Z 轴颜色
				...
				this.xAxis = new THREE.Line(xGeometry, xAxisMaterial);
				...
				this.yAxis = new THREE.Line(yGeometry, yAxisMaterial);
				...
				this.zAxis = new THREE.Line(zGeometry, zAxisMaterial);
				...
				*/
				...
				// 	document.getElementById('bounding_box_width').addEventListener('input', this.updateCubeSize);
				// 	document.getElementById('bounding_box_depth').addEventListener('input', this.updateCubeSize);
				...
				this.add_listener(document.getElementById('bounding_box_depth'), 'input', this.updateCubeSize)
				...
				this.add_listener(document.getElementById('modify_scale_cylinder'),"input", this.modify_scale_cylinder)
	...
	*/
	enable_boundingboxcontrol(){
		this.bounding_box_dragControls.enabled = true;
	}
	// 创建坐标轴的函数
	createAxes() {
		//console.log('calling createaxes', this.axes_exist)
		if (this.axes_exist == 1) return;
		...
		const yDirection = new THREE.Vector3(0, 1, 0); // X 轴方向
		const yLength = 15; // 箭头长度
		const yHeadLength = 0.5; // 箭头头部长度
		const yHeadWidth = 0.2; // 箭头头部宽度
		this.yAxis = new THREE.ArrowHelper(yDirection, yOrigin, yLength, 0x00ff00, yHeadLength, yHeadWidth);
		this.add_mesh(this.scene, [this.yAxis])


		const zOrigin = new THREE.Vector3(0, 0, -7.5); // 箭头起点
		const zDirection = new THREE.Vector3(0, 0, 1); // X 轴方向
	...
	removeAxes() {
		//console.log("calling remove axes", this.axes_exist)
		if (this.axes_exist == 0) return;
	...
	add_scale_cylinder() {
		...
		if (this.scale_cylinder_exist == 1){
			//this.scene.scale_cylinder.visible = true;
			this.add_mesh(this.scene,[this.scale_cylinder])
			return
		}
		else{
			const scale_cylinderGeometry = new THREE.CylinderGeometry(0.2, 0.2, 0.2, 32);
			...
			this.add_listener(this.scale_cylinder_dragControls, 'dragstart', function (event) {
				//if (event.pointerType === 'mouse' && event.button === 0) {
				//	event.object.material.opacity = 0.5;
				//} else {
				//	// 如果不是左键拖拽，则取消拖拽
			...
			this.add_listener(this.scale_cylinder_dragControls, 'dragend', function (event) {
				//event.object.material.opacity = 1;
				//this.save_bounding_box_pos.copy(this.bounding_box.position)
	...
	modify_scale_cylinder() {
		...
		const new_cylinder_height = parseFloat(document.getElementById('modify_scale_cylinder').value) || 0;
	...
	updateCubeSize() {
		...
		const new_width = parseFloat(document.getElementById('bounding_box_width').value) || 0.5;
		const new_depth = parseFloat(document.getElementById('bounding_box_depth').value) || 0.5; // 深度
		const new_height = parseFloat(document.getElementById('bounding_box_height').value) || 0.5; // 高度
		document.getElementById('bounding_box_width_value').textContent = new_width;
		document.getElementById('bounding_box_depth_value').textContent = new_depth;
    	...
    	this.bounding_box.geometry = new THREE.EdgesGeometry(new THREE.BoxGeometry(new_width, new_depth, new_height));
		this.bounding_box.position.copy(bounding_box_current_position); 


		//this.bounding_box.scale.set(scale_width, scale_depth, scale_height);
	...
	setup_prompt_original() {
		...
		<h1><hr/></h1>
		<p style="margin-bottom: 10px;"><span style="font-size: 1.5em;">Facing the bicycle, place a trash bin 0.5m in diameter and 0.75m in height on the floor. It shoule be located 1m away directly to the right of the bed.</span></p>
		<!--
		<p style="margin-bottom: 10px;">There is a green 3D bounding box in the 3D scene above.</p>
		<p style="margin-bottom: 10px;">To change the size of bounding box, you need to drag the control bar below, i.e. 'Bounding Box Width', 'Bounding Box Depth', 'Bounding Box Height'.</p>
		<p style="margin-bottom: 10px;">To change the position of bounding box, you may either drag it by mouse or drag the control bar below, i.e. 'Bounding Box x Position','Bounding Box y position', 'Bounding Box z Position'.</p>
		<p style="margin-bottom: 10px;">To change the Rotation Angle of Bounding box in XoY plane, drag the control bar corresponds to 'Bounding Box Rotation Angle.'</p>
		<p style="margin-bottom: 10px;">To better understand coordinate system and coordinate axes, you can click 'Show Axis' for more information. The color of Axis is consistant to the title of control bar. To remove Axes, simply click 'Hide Axis'.</p>
		-->
		<div id="controls">
			<div style="display: flex; justify-content: space-between; height: 45vh;">
				<!-- 左侧内容 -->
				<div style="width: 70%; padding: 20px; background-color: #f0f0f0; display: flex; flex-direction: column; justify-content: flex-start;">			
					<div class="tickmarks" style="position: relative; width: 100%; display: flex; justify-content: space-between; margin-bottom: 10px;">
						<span>0</span>
						<span>0.1</span>
						<span>0.2</span>
						<span>0.3</span>
					...
					<label for="bounding_box_width" style="color: #ff0000;">Bounding Box Width: <span id="bounding_box_width_value">0.5</span></label>
					<input type="range" id="bounding_box_width" min="0" max="3" step="0.05" value="0.5" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_depth" style="color: #00ff00;">Bounding Box Depth: <span id="bounding_box_depth_value">0.5</span></label>
					<input type="range" id="bounding_box_depth" min="0" max="3" step="0.05" value="0.5" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_height" style="color: #0000ff;">Bounding Box Height: <span id="bounding_box_height_value">0.5</span></label>
					<input type="range" id="bounding_box_height" min="0" max="3" step="0.05" value="0.5" style="width: 100%; margin-top: 10px;">
					<div class="tickmarks" style="position: relative; width: 100%; display: flex; justify-content: space-between; top: 10px;">
						<span class="tick" style="left: 0%;">0</span>
						<span class="tick" style="left: 5%;">18</span>
						<span class="tick" style="left: 10%;">36</span>
						<span class="tick" style="left: 15%;">54</span>
						<span class="tick" style="left: 20%;">72</span>
						<span class="tick" style="left: 25%;">90</span>
						<span class="tick" style="left: 30%;">108</span>
						<span class="tick" style="left: 35%;">126</span>
						<span class="tick" style="left: 40%;">144</span>
						<span class="tick" style="left: 45%;">162</span>
						<span class="tick" style="left: 50%;">180</span>
						<span class="tick" style="left: 55%;">198</span>
						<span class="tick" style="left: 60%;">216</span>
						<span class="tick" style="left: 65%;">234</span>
						<span class="tick" style="left: 70%;">252</span>
						<span class="tick" style="left: 75%;">270</span>
						<span class="tick" style="left: 80%;">288</span>
						<span class="tick" style="left: 85%;">306</span>
						<span class="tick" style="left: 90%;">324</span>
						<span class="tick" style="left: 95%;">342</span>
						<span class="tick" style="left: 100%;">360</span>
					</div><br>
					<label for="bounding_box_rotation_angle">Bounding Box Rotation Angle:</label>
					<input type="range" id="bounding_box_rotation_angle" min="0" max="360" step = "1" value="0" style="width: 100%; margin-top: 10px;"><br>
					<p id="display_selected_object_info"></p><br>	
				</div>
				<!-- 右侧内容 -->
				<div style="width: 30%; padding: 20px; background-color: #e0e0e0; display: flex; flex-direction: column; justify-content: flex-start;">
					<div class="tickmarks" style="position: relative; width: 100%; display: flex; justify-content: space-between; margin-bottom: 10px;">
						<span>-5</span>
						<span>-4</span>
						<span>-3</span>
					...
					<label for="bounding_box_center_x_position" style="color: #ff0000;">Bounding Box x Position</label>
					<input id="bounding_box_center_x_position" type="range" min="-5" max="5" step="0.05" value="0" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_center_y_position" style="color: #00ff00;">Bounding Box y Position</label>
					<input id="bounding_box_center_y_position" type="range" min="-5" max="5" step="0.05" value="0" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_center_z_position" style="color: #0000ff;">Bounding Box z Position</label>
					<input id="bounding_box_center_z_position" type="range" min="-5" max="5" step="0.05" value="0" style="width: 100%; margin-top: 10px;">
					<label for="scale_cylinder_diameter">Scale Cylinder Height</label>
					<input type="number" id="modify_scale_cylinder" value="0.5" min="0" max="10" step = "0.1">
					<label for="scale_cylinder_rotation_angle">Scale Cylinder Rotation Angle:</label>
					<input type="range" id="scale_cylinder_rotation_angle" min="0" max="360" step = "1" value="0" style="width: 100%; margin-top: 10px;">
				</div>
			</div>
			<br>
			<!--
			<p style="margin-bottom: 10px;">There is also a red cylinder named 'Scale Cylinder' in the 3D scene. You can change its height to measure distance in 3D scene. Then you are allowed to change its position by mouse control as well as change its Rotation Angle by control bar.</p>
			<p style="margin-bottom: 10px;">Once you finish measure distance, you can click 'Remove Scale Cylinder' and continue adjust bounding box as you wish. Mention that once the Scale Cylineder been removed, it not longer can be added to the scene</p>
			-->
			<!--
			<label for="scale_cylinder_Height">Scale Cylinder Height</label>
			<input type="number" id="modify_scale_cylinder" value="0.5" min="0" max="10" step = "0.1"><br>
			<label for="scale_cylinder_rotation_angle">Scale Cylinder Rotation Angle:</label>
			<input type="range" id="scale_cylinder_rotation_angle" min="0" max="360" step = "1" value="0" style="width: 100%; margin-top: 10px;">
			-->
			<button id="show_axis">Show Axis</button>
			<button id="hide_axis">Hide Axis</button>
			<button id="show_scale_cylinder">Show Scale Cylinder</button>
			<button id="hide_scale_cylinder">Hide Scale Cylinder</button><br>
    	</div>
		<div>
		...
		<div>
			...
			<p style="display:inline; font-size:25px"> Describe the <span style="color:blue; font-size:25px">location and orientation</span> (<b>"context"</b>) picked in Step 2 with a sentence. The description should locate the <b>unique</b> position in the scene. <b>Ambiguous or wrong description will be <span style="color:red; font-size:25px">rejected</span></b>.</p>
		</div>
		<div>
		...
		<div>
			<label style="font:45px bold">Step 4</label>
			<p style="display:inline; font-size:25px"> Based on the <b>"context"</b> in Step 2&3, ask a question about the 3D scene and answer it, then click 'Submit'. <b> Questions that can be answered without considering the context will be <span style="color:red; font-size:25px">rejected</span></b></p>.
		</div>
		<div>
		...
		this.add_listener(this.btn.sqa_submit, "click", this.on_click_btn_sqa_submit_original);
	}
	on_click_btn_sqa_submit_original() {
		...
		if (!situation || !question || !answer) {
			alert("Please fill in situation, question and answer!");
		}
		else {
			...
			window.xhr_post(JSON.stringify(record), "/apps/database/sqa3d/save/sqa_collection={0}".format(this.resources.sqa_db_collection_name)).then(() => {
				...
				alert(["Recorded.", situation, question, answer].join(' '));
			});
		}
	}
	...
	on_click_label(id) {
		return function () {
			...
			if (this.selected_id == 0){
				...
				this.selected_object_info = this.scenegraph[this.resources.scene_id]['objects_info'][this.selected_id - 1]
				...
				this.selected_object_depth = Math.round(this.selected_object_size[1] * 100)/100
				this.selected_object_height = Math.round(this.selected_object_size[2] * 100)/100
				display_selected_object_info.textContent = "Selecting " + this.selected_object_label + ". " + "The corresponding Width, Depth and Height are " + this.selected_object_width + ", " + this.selected_object_depth + ", " + this.selected_object_height
				//console.log(this.selected_object_label, this.selected_object_size)
			}
			//this.object_info = this.scenegraph[this.resources.scene_id]['objects_info']
			//console.log("scene graph information", this.scenegraph)
			//console.log("selected scene" + this.resources.scene_id)

			//const fs = require('fs');
			//var scene_graph_dir = '/home/wangtianxu/SceneVerse_Dataset/scenegraph/ScanNet/' + this.resources.scene_id + '/objects.json'
			// fetch('path/to/your/file.json')
			// 	.then(response => response.json())  // 解析 JSON 数据
			// 	.then(data => {
			// 		console.log(data);  // 使用 JSON 数据
			// 	})
			// 	.catch(error => {
	...
	add_listener(element, event, callback, argument = null) {
		console.log(element, event, callback)

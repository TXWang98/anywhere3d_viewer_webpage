...
class MeshViewer {
	...
	*/
	...
	callCylinder(){
		...
		//2: exist but hide 
		if (this.scale_cylinder_exist == 0){
	...
	modify_referring_expressions(){
		...
		if (input_referring_expressions_field.value === "") {
            input_referring_expressions_field.value = placeholdertext;
        }
		...
		<div style="display: flex; justify-content: space-between;">
			<div style="width: 100%; padding: 20px; background-color: #f0f0f0; display: flex; flex-direction: column; justify-content: flex-start; overflow: auto">			
				...
				<!--div style="display: flex; justify-content: center; width: 100%;">
					<button id="show_scale_cylinder" style="text-align: left;">Show Cylinder</button>
					<button id="hide_scale_cylinder" style="text-align: left;">Hide Cylinder</button>
				</div-->
				<div style="display: flex; justify-content: space-between; width: 100%;">
					<p><span style="font-size: 1.2em;">Bounding Box</span></p>
					<button id="call_axes_button" style="width: 70px; height: 35px;" >Axes</button>
				</div>
				...
				<div style="display: flex; justify-content: space-between; width: 100%;">
					...
					<button id="call_cylinder_button" style="width: 70px; height: 35px;" >Cylinder</button>
				</div>
				<div style="display: flex; align-items: center; width: 100%;">
					<label for="scale_cylinder_diameter" style="width: 30%;">L: <span id="scale_cylinder_bar_height">0.5</span></label>
					<input type="range" id="modify_scale_cylinder" min="0" max="10" step = "0.01" value="0.5" style="flex-grow: 1;">		
				</div>
				<div style="display: flex; align-items: center; width: 100%;">
	...
	setup_prompt_original() {
		...
		<p style="margin-bottom: 10px;">
			<span style="font-size: 1.5em;"><b>Referring Expressions:</b> 	
				<input type="text" class="form-control" id="new_referring_expressions" placeholder='Facing the bicycle, place a trash bin 0.5m in diameter and 0.75m in height on the floor. It should be located 1m away directly to the right of the bed.'</p>
			</span>
		</p>
		<!--
		...
		<div>
			...
			<p style="display: inline; font-size: 25px"> Explore the 3D scene above. Click 'Tutorial' for details on how to interact with the scene and objects. Also, you can know how to use <b>Bounding Box</b> tool and <b>Cylinder</b> tool. </p>
		</div>
		<h2><hr/></h2>
		...
		<div>
			...
			<p style="display: inline; font-size: 25px"> Check the 'Referring Expressions' above. According to the referring expressions, determine the <span style="color:blue; font-size:25px">position</span> as well as the <span style="color:blue; font-size:25px">size</span> of the Bounding Box.</p>
		</div>
		<h2><hr/></h2>
		...
		<div>
			...
			<p style="display:inline; font-size:25px"> If you are unable to determine the position or size of the bounding box based on the referring expressions, please modify it. Then, determine the <span style="color:blue; font-size:25px">position</span> and <span style="color:blue; font-size:25px">size</span> of the bounding box according to your <b>modified</b> referring expressions. Please ensure that the modified referring expressions do <b>not</b> differ significantly from the original ones. Note that this step is <b>not</b> mandatory!  </p>
		</div>
		<!--div>
			<input type="text" class="form-control" id="new_referring_expressions" placeholder="Place a trash can 0.5m in width, length and height on the floor in the corner besides the door." required=""></input>
		</div-->
		<h2><hr/></h2>
		...
		<div>
			...
			<p style="display:inline; font-size:25px"> Once you determine the position and size of the bounding box according to the referring expressions, please do <b>not</b> change the camera view and bounding box, then click 'Submit'. <br><span style="color:red; font-size:25px">Rejection Alert:</span> Misalignment between the referring expressions and the bounding box will result in rejection. </p>
		</div>
		<!--div>
		...
		<h1><hr/></h1>
		<button id="btn_anywhere3D_submit" class="w-100 btn btn-primary btn-lg" type="submit">Submit</button>
		`;
	...
	on_click_btn_anywhere3D_submit_original() {
		...
		if (input_referring_expressions_field.value === ''){
			record.referring_expressions = input_referring_expressions_field.placeholder
		}else{
			record.referring_expressions = input_referring_expressions_field.value
		}
		
		record.scene_id = this.resources.scene_id;
		
		//const boundingBoxGeometry = this.bounding_box.geometry.parameters;
		/*
		...
		*/
		...
		record.bounding_box_rotation_angle = this.bounding_box_rotation_angle
		...
		*/
		...
		console.log(record)
		/*
		const width = boundingBoxGeometry.width;
		const height = boundingBoxGeometry.height;
		const depth = boundingBoxGeometry.depth;
		*/
		//record.bounding_box_width = boundingBoxGeometry.width;
		//record.bounding_box_length = boundingBoxGeometry.heig
		...
		*/
		...
		window.xhr_post(JSON.stringify(record), "/apps/database/sqa3d/save/sqa_collection={0}".format(this.resources.sqa_db_collection_name)).then(() => {
			...
			alert(["Recorded.", record.bounding_box_width, record.bounding_box_length, record.bounding_box_height, record.bounding_box_rotation_angle, record.bounding_box_xpos, record.bounding_box_ypos, record.bounding_box_zpos].join(' '));
		});

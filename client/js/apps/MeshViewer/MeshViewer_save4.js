...
class MeshViewer {
	...
	init(params) {
		...
		fetch(path.join("/apps/resource/datasettype/", this.resources.datasettype)).then(response => {
			if (!response.ok) {
				throw new Error('Network response was not ok')
			}
			//console.log("response", response)
	...
	*/
	...
	modify_scale_cylinder() {
		...
		const new_cylinder_height = parseFloat(document.getElementById('modify_scale_cylinder').value) || 0.5;
		document.getElementById('scale_cylinder_bar_height').textContent = new_cylinder_height;
	...
	updateCubeSize() {
		...
		const new_length = parseFloat(document.getElementById('bounding_box_length').value) || 0.5; // 深度
	...
	setup_prompt_original() {
		...
		<p style="margin-bottom: 10px;">To change the size of bounding box, you need to drag the control bar below, i.e. 'Bounding Box Width', 'Bounding Box Length', 'Bounding Box Height'.</p>
		...
		<div id="controls">
			<div style="display: flex; justify-content: space-between; height: 45vh;">
				...
				<div style="width: 70%; padding: 20px; background-color: #f0f0f0; display: flex; flex-direction: column; justify-content: flex-start;">			
					...
					<input type="range" id="bounding_box_width" min="0" max="4" step="0.01" value="0.5" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_length" style="color: #00ff00;">Bounding Box Length: <span id="bounding_box_length_value">0.5</span></label>
					<input type="range" id="bounding_box_length" min="0" max="4" step="0.01" value="0.5" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_height" style="color: #0000ff;">Bounding Box Height: <span id="bounding_box_height_value">0.5</span></label>
					<input type="range" id="bounding_box_height" min="0" max="3" step="0.01" value="0.5" style="width: 100%; margin-top: 10px;">
				...
				<div style="width: 30%; padding: 20px; background-color: #e0e0e0; display: flex; flex-direction: column; justify-content: flex-start;">
					...
					<input id="bounding_box_center_x_position" type="range" min="-5" max="5" step="0.01" value="0" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_center_y_position" style="color: #00ff00;">Bounding Box y Position</label>
					<input id="bounding_box_center_y_position" type="range" min="-5" max="5" step="0.01" value="0" style="width: 100%; margin-top: 10px;">
					<label for="bounding_box_center_z_position" style="color: #0000ff;">Bounding Box z Position</label>
					<input id="bounding_box_center_z_position" type="range" min="-5" max="5" step="0.01" value="0" style="width: 100%; margin-top: 10px;">
					<label for="scale_cylinder_diameter">Scale Cylinder Height: <span id="scale_cylinder_bar_height">0.5</span></label>
					<input id="modify_scale_cylinder" type="range" min="0" max="10" step = "0.01" value="0.5" style="width: 100%; margin-top: 10px;">

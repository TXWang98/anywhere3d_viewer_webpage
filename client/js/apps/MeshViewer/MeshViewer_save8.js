...
class MeshViewer {
		...
		Promise.all(promises).then(values => {
			...
			Promise.all(new_promises).then(new_values => {
				...
				*/
				...
				this.all_referring_expressions_placeholder = [
				...
				this.cur_referring_expressions_cnt = 0
				//this.cur_referring_expressions_value = ''
				//this.cur_referring_expressions_placeholder = 
				document.getElementById('new_referring_expressions').placeholder = this.all_referring_expressions_placeholder[this.cur_referring_expressions_cnt]
				...
				this.add_listener(document.getElementById('annotator_id'), "focus", this.modify_annotator_id)
	...
	*/
	...
	modify_annotator_id(){
		var annotator_id_field = document.getElementById('annotator_id');
		//var placeholdertext = input_referring_expressions_field.placeholder;
		if (annotator_id_field.value === "") {
            annotator_id_field.value = annotator_id_field.placeholder;
			//this.cur_referring_expressions_value = this.cur_referring_expressions_placeholder;
        }else{
			if (this.annotator_id == ''){
				this.annotator_id = annotator_id_field.value
			}
		}
	...
	setup_prompt_original() {
		...
		<p style="margin-bottom: 10px;">
			<span style="font-size: 1.5em;"><b>Annotator ID:</b> 	
				<input type="text" class="form-control" id="annotator_id" placeholder='Please enter your name/id' value=''</p>
			</span>
		</p>
		<p style="margin-bottom: 10px;">
			<span style="font-size: 1.5em;"><b>Referring Expressions:</b> 	
				<input type="text" class="form-control" id="new_referring_expressions" placeholder='' value=''</p>
			</span>
		</p>
		<!--
	...
	on_click_btn_anywhere3D_submit_original() {
		...
		var input_annotator_id_field = document.getElementById("annotator_id");
		if (input_annotator_id_field.value === ''){
			record.annotator_id = input_annotator_id_field.placeholder
		}else{
			record.annotator_id = input_annotator_id_field.value
		}
		...
		*/
		...
		window.xhr_post(JSON.stringify(record), "/apps/database/sqa3d/save/sqa_collection={0}".format(this.resources.sqa_db_collection_name)).then(() => {
			...
			alert(["Recorded.", record.referring_expressions,"with bounding box size",record.bounding_box_width, record.bounding_box_length, record.bounding_box_height].join(' '));
		});
		...
		this.cur_referring_expressions_cnt += 1;
		if (this.cur_referring_expressions_cnt > 8){
			input_referring_expressions_field.placeholder = this.all_referring_expressions_placeholder[-1]
		}else{
			input_referring_expressions_field.placeholder = this.all_referring_expressions_placeholder[this.cur_referring_expressions_cnt]
		}

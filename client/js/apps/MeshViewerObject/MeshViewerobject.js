...
class MeshViewerobject {
	...
	*/
	...
	start_new_annotation(){
		...
		document.getElementById('original_referring_object').textContent = 'Origianl Referring Object:' + this.all_answers[this.cur_referring_expressions_cnt]
	...
	load_annotation_cache(RE_id){
		...
		window.xhr_post(JSON.stringify(request_for_annotation), "/apps/database/sqa3d/load/anywhere3D_collection={0}".format(this.resources.sqa_db_collection_name)).then(results => {
			...
			if (this.querying_annotation_db_data['flag'] == true){
				...
				document.getElementById('original_referring_object').textContent = ''
	...
	*/
	...
	setup_prompt_original() {
		...
		<p style="margin-bottom: 10px;">
			...
			<div>
				...
				<span id='original_referring_object'></span></br>
	...
	get_resource(params) {
		...
		this.resources.ref_exp_graphs = "ref_exp_graphs_object.json"
...
window.MeshViewerobject = MeshViewerobject;

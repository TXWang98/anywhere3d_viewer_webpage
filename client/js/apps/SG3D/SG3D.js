...
import RootUI from './view/RootUI.js';
import ViewUI from "./view/ViewUI.js";
import WindowManager from "../Common/WindowManager.js"
import ControlContent from './view/ControlContent.js'
import InfoContent from './view/InfoContent.js'
import Stats from "../../lib/utils/stats.min.js";
import * as utils from "../Common/utils.js";
import * as nyuv2 from "../Common/NYUv2_colors.js";
...
class SG3D {
	...
	init(params) {
		...
		.then(data => {
			this.all_referring_expressions = data["referring_expressions"]//.replace(/\n/g, "<br>")
			//console.log("type!!!!", typeof this.all_referring_expressions, data["referring_expressions"])
		...
		*/
		...
		Promise.all(promises).then(values => {
			...
			// console.log(this.scene_geometry)
			this.calculate_xyaverage_zmin()
			console.log(this.scene_geometry.average_x, this.scene_geometry.average_y, this.scene_geometry.minimum_z)
	...
	*/
	...
	calculate_xyaverage_zmin(){
		const point_positions = this.scene_geometry.attributes.position.array; // Float32Array
		let sumX = 0, sumY = 0;
		let minZ = 1000000;
		const numPoints = point_positions.length / 3; // 点的数量

		for (let i = 0; i < point_positions.length; i += 3) {
			const x_coor = point_positions[i];
			const y_coor = point_positions[i + 1];
			const z_coor = point_positions[i + 2];
		...
		const meanXcoor = sumX / numPoints;
		const meanYcoor = sumY / numPoints;
		this.scene_geometry.average_x = Math.round(meanXcoor * 100) / 100
		this.scene_geometry.average_y = Math.round(meanYcoor * 100) / 100;
		this.scene_geometry.minimum_z = Math.round(minZ * 100)/ 100;

		console.log("computing average_x, average_y, min_z", this.scene_geometry.average_x, this.scene_geometry.average_y, this.scene_geometry.minimum_z)

		//return new Array(meanXcoor, meanYcoor, minZ);
	}
	...
	load_annotation_db(){
		...
		console.log(this.resources.sqa_db_collection_name)
		window.xhr_post(JSON.stringify(request_for_annotation), "/apps/database/sqa3d/load/anywhere3D_collection={0}".format(this.resources.sqa_db_collection_name)).then(results => {
			...
			if (this.annotation_db_data['flag'] == true){
				...
				document.getElementById('original_referring_expressions').innerHTML = 'Original Referring Expressions: ' + this.all_referring_expressions[this.cur_referring_expressions_cnt].replace(/\n/g, "<br>")
	...
	load_annotation_cache(RE_id){
		...
		request_for_annotation.scene_id = this.resources.scene_id
		request_for_annotation.cur_referring_expressions_cnt = this.cur_referring_expressions_cnt
		//console.log("hihihihi", this.resources.sqa_db_collection_name)
		window.xhr_post(JSON.stringify(request_for_annotation), "/apps/database/sqa3d/load/anywhere3D_collection={0}".format(this.resources.sqa_db_collection_name)).then(results => {
			//console.log(results)
	...
	*/
		...
		<div style="display: flex; justify-content: space-between;">
			<div style="width: 100%; padding: 20px; background-color: #f0f0f0; display: flex; flex-direction: column; justify-content: flex-start; overflow: auto">			
				...
				<div style="display: flex; align-items: center; width: 100%;">
					...
					<input type="range" id="bounding_box_width" min="0" max="6" step="0.01" value="0.5" style="flex-grow: 1;">
				</div>
				<div style="display: flex; align-items: center; width: 100%;">
					<label for="bounding_box_length" style="color: #228B22; width: 30%;">L: <span id="bounding_box_length_value">0.5</span></label>
					<input type="range" id="bounding_box_length" min="0" max="6" step="0.01" value="0.5" style="flex-grow: 1;">
				</div>
				<div style="display: flex; align-items: center; width: 100%;">
					<label for="bounding_box_height" style="color: #0000ff; width: 30%;">H: <span id="bounding_box_height_value">0.5</span></label>
					<input type="range" id="bounding_box_height" min="0" max="4" step="0.01" value="0.5" style="flex-grow: 1;">
				</div>
				<div style="display: flex; align-items: center; width: 100%;">
	...
	render_canvas_label(x, y) {
		...
		this.bounding_box_width = this.intersected_object_size[0];
		this.bounding_box_length = this.intersected_object_size[1];
		this.bounding_box_height = this.intersected_object_size[2];
		...
		//reset bounding box xpos, ypos, zpos
		this.bounding_box.position.set(this.intersected_object_center[0], this.intersected_object_center[1], this.intersected_object_center[2]);
		document.getElementById('bounding_box_center_x_position').value = 0;
		document.getElementById('bounding_box_center_y_position').value = 0;
		document.getElementById('bounding_box_center_z_position').value = 0;
		this.bounding_box_bar_x = 0;
	...
	render_label_list() {
		...
		for (let i = 0; i < this.resources.scene_object_name.length; i++) {
			...
			if (this.resources.datasetname == 'scannet'){
				...
				this.selected_object_position = this.selected_object_info['position']
				...
				this.selected_object_xpos = Math.round(this.selected_object_position[0] * 100) / 100
				this.selected_object_ypos = Math.round(this.selected_object_position[1] * 100) / 100
				this.selected_object_zpos = Math.round(this.selected_object_position[2] * 100) / 100

				this.object_mesh[i].center = new Array(this.selected_object_xpos, this.selected_object_ypos, this.selected_object_zpos)
				this.object_mesh[i].size = new Array(this.selected_object_width, this.selected_object_length, this.selected_object_height)
				...
				this.object_mesh[i].center = new Array(this.selected_object_xpos + this.scene_geometry.average_x, this.selected_object_ypos + this.scene_geometry.average_y, this.selected_object_zpos + this.scene_geometry.minimum_z)
				this.object_mesh[i].size = new Array(this.selected_object_width, this.selected_object_length, this.selected_object_height)
				...
				//this.object_mesh[i].center = new Array(this.selected_object_xpos, this.selected_object_ypos, this.selected_object_zpos + this.scene_geometry.minimum_z)
	...
	get_resource(params) {
		...
		this.resources.ref_exp_graphs = "ref_exp_graphs_SG3D.json"
		...
		if (this.window_mesh.is_mouse_in_model_panel()) {
			...
			if (intersected.length > 0) {
				if (!(ignore_list.includes(intersected[0].object.material.emissive.getHex()))) {
					...
					this.intersected_object_center = this.intersected[0].center
					this.intersected_object_size = this.intersected[0].size
					//console.log("position and size", this.intersected[0].center, this.intersected[0].size)
	...
	set_mesh(geometry, material) {
		...
		if (this.resources.datasetname == 'scannet'){
			...
			//console.log("this.scene_geometry.center", this.scene_geometry_center.x, this.scene_geometry_center.y)
